...
module Internal = struct
  let validate (facts : Cfg_validation_facts.Graph.t) =
    ...
    if Node.Map.equal (fun () () -> true) expected_facts actual
    then Ok ()
...
module Z3 = struct
  let code (facts : Cfg_validation_facts.Graph.t) =
    ...
    let id_gen = Cfg_z3.Label_id_gen.create nodes in
    let id label = Cfg_z3.Label_id_gen.get_id id_gen ~key:label in
